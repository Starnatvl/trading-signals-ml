...
Система поддерживает несколько режимов, управляемых через файл `.env`. В зависимости от переменных окружения воркер может получать данные из разных источников и отправлять сигналы в разные адресата.

**Переменные, влияющие на режим:**

- `USE_MOCK=true` – принудительно переключает воркер в режим работы с локальным мок-сервером (`http://localhost:3000`), игнорируя production-переменные.
- `SIGNAL_TO_MOCK=true` – отправляет сигналы на локальный мок-сервер, даже если данные получаются с реального API.
- `API_BASE_URL`, `API_USERNAME`, `API_PASSWORD` – параметры для подключения к реальному API платформы (замените на реальные данные, полученные от заказчика).
- `SIGNAL_SOURCE` – определяет источник сигнала (`ml_shadow` или `ml`). По умолчанию `ml_shadow`.
- `POLL_INTERVAL` – интервал опроса в секундах (по умолчанию 60 в production, 5 в demo).
- `LOG_FEATURES=true` – включает сохранение сырых окон признаков в CSV-файлы (папка `integration/logs/`).

Ниже описаны основные сценарии использования.

#### 📌 Режим 1 – только локальный JSON (данные и сигналы в мок)

Используется для тестирования модели на заранее подготовленных данных (JSON) без подключения к реальному API.

```ini
USE_MOCK=true
LOG_FEATURES=true   # опционально
```

Воркер берёт данные с `http://localhost:3000` (мок-сервер), сигналы отправляет туда же. Мок-сервер должен быть запущен отдельно (см. раздел «Быстрый старт»).

#### 📌 Режим 2 – данные с реального API, сигналы в локальный мок

Позволяет получать реальные данные с платформы, но при этом отправлять сигналы в локальный мок-сервер для отладки и сбора статистики. Идеально для ночного сбора данных и проверки логики без влияния на production.

```ini
API_BASE_URL=https://your-platform.com
API_USERNAME=dataset
API_PASSWORD=ваш_пароль
SIGNAL_TO_MOCK=true
LOG_FEATURES=true
```

В этом режиме воркер запрашивает данные с реального API, используя учётные данные пользователя `dataset` (только чтение). Сигналы отправляются на `localhost:3000`. Ошибки авторизации при отправке не возникают.

#### 📌 Режим 3 – данные с реального API, сигналы в реальное API (теневой режим)

Предпродакшн-режим. Сигналы отправляются в реальное API с источником `ml_shadow`. Требуется учётная запись с правами `operator`.
...
Платформа принимает сигналы, но, как правило, не использует их для исполнения сделок – только логирует.

#### 📌 Режим 4 – полноценный продакшн (сигналы с источником `ml`)

Аналогичен режиму 3, но с `SIGNAL_SOURCE=ml`. Используется для реальной торговли. На данном этапе не требуется.
...
API_PASSWORD=пароль
SIGNAL_SOURCE=ml
LOG_FEATURES=false   # обычно в проде логирование отключают
```

#### ✅ Проверка логов при запуске

При запуске воркер выводит подробную информацию о текущем режиме. Например, для режима 2 вы увидите:

```
✅ Загружены переменные из .env
[2026-03-09 20:00:01.123] 🚀 Worker запущен в режиме: PRODUCTION
[2026-03-09 20:00:01.123]    Данные: https://your-platform.com (авторизация: dataset)
[2026-03-09 20:00:01.123]    Сигналы: http://localhost:3000 (локальный мок-сервер, без авторизации)
[2026-03-09 20:00:01.123]    Интервал опроса: 60 сек
[2026-03-09 20:00:01.123]    📁 Логирование входных окон включено (папка logs/)
```

Это позволяет сразу убедиться, что конфигурация задана верно.

#### ⚠️ Важное замечание

- Файл `.env` **не должен** попадать в репозиторий – добавьте его в `.gitignore`.
- Логи (CSV-файлы) также не следует коммитить – папка `integration/logs/` уже добавлена в `.gitignore`.
